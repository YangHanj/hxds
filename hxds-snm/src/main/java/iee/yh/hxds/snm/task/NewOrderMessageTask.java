package iee.yh.hxds.snm.task;

import cn.hutool.core.collection.ListUtil;
import cn.hutool.core.map.MapUtil;
import com.rabbitmq.client.*;
import iee.yh.common.exception.HxdsException;
import iee.yh.hxds.snm.entity.NewOrderMessage;
...
public class NewOrderMessageTask {
    ...
    final int ttl = 1 * 60 * 1000; // 新订单消息缓存过期时间1分钟
    final String exchangeName = "new_order_private";

    private Logger log = LoggerFactory.getLogger(NewOrderMessageTask.class);
    ...
    public void sendNewOrderMessage(ArrayList<NewOrderMessage> list){
        try (
            ...
            for (NewOrderMessage message : list){
                ...
                // 创建消息属性对象
                AMQP.BasicProperties properties = new AMQP.BasicProperties().builder().contentEncoding("UTF-8").headers(map).expiration(ttl + "").build();
                ...
                log.info(message.getUserId() + "的新订单消息发送成功");
            }
        }catch (Exception e){
            log.error("执行异常", e);
